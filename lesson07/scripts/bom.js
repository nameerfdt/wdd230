...
// create event listener when button is clicked
...
// Create the displayList defined function that receives one parameter named item. The item parameter is the input.
function displayList(item) {
    // create a li (list) element
    let li = document.createElement('li');
    // create a deliete button
    let deleteButton = document.createElement('button');
    // let word = input.value; not needed because item is input
    //  create variable to uppercase first letter and lowercase all others from the input (item)
    // let capWord = item.charAt(0).toUpperCase() + item.slice(1).toLowerCase();
    // populate the list elements textContent or innerHTML with the input value. the input value is item
    li.textContent = item;
...
// Define the setChapterList function to set the localStorage item that you have already named. Use JSON.stringify() to stringify the array. This is the KEY.
